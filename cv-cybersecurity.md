# Abdullah Khan
**Software Engineer — Cyber Security Specialization**
Islamabad, Pakistan | +92 331 383 0800 | abdullahbaloch070@gmail.com | linkedin.com/in/abdullahkhan07

## Summary
Software engineer pursuing an MS in Cyber Security (2024–2026) with practical, production experience implementing secure system architecture — role-based access control, audit logging, multi-tenant data isolation, and cloud infrastructure hardening. Looking to move into security-focused engineering roles that combine hands-on development with security depth.

## Skills
- **Security Fundamentals:** Access Control (RBAC), Audit Logging, Multi-tenant Data Isolation, Secure API Design
- **Cloud & Infrastructure:** AWS (EC2, S3), CI/CD Pipeline Security, Docker
- **Engineering:** NestJS, Node.js, .NET Core/C#, PostgreSQL, GraphQL, TypeORM
- **Currently studying:** Cyber Security (MS, FAST-NUCES)

## Professional Experience
**Engineering Manager / Senior Software Engineer**
Xperion | Nov 2023 – Present | Islamabad, Pakistan
- Architected multi-tenant microservices with role-based access control and comprehensive audit logging using NestJS and TypeORM.
- Managed AWS infrastructure (EC2, S3) and CI/CD deployment pipelines, ensuring 99.9% uptime.
- Designed GraphQL gateways governing data access across distributed services, with consistency and access-control considerations across tenants.

**Software Engineer**
Thathal Information and Technological Solutions | Feb 2023 – Jun 2023 | Pakistan
- Decomposed a monolithic backend into microservices, reducing blast radius and enabling isolated scaling per service.

## Education
- **MS Cyber Security** – National University of Computer and Emerging Sciences (FAST-NUCES) | 2024 – 2026 (in progress)
- **BS Computer Science** – National University of Computer and Emerging Sciences (FAST-NUCES) | 2017 – 2023
